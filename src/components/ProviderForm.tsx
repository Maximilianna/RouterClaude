...
export default function ProviderForm({ initial, onSave, onCancel, onToast }: Props) {
  ...
  function handleApiModeChange(mode: ApiMode) {
    setApiMode(mode);
    // Auto-fill URL from preset if one is selected
  ...
  async function handleDiscover() {
    ...
    // Preset: always use openaiUrl for discovery; Custom: use user-entered apiUrl
    const preset = PROVIDER_PRESETS.find((p) => p.name === selectedPreset || p.name === name);
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        ...
        <div className="relative">
          <input
            className="w-full border border-gray-200 rounded-xl px-4 py-2.5 pr-10 text-sm font-mono focus:border-blue-400 focus:ring-2 focus:ring-blue-100 transition-all duration-200 outline-none"
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2 mb-2">
          {tags.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 px-2.5 py-1 bg-blue-50 text-blue-600 text-xs rounded-full border border-blue-100"
            >
              {tag}
              <button
                type="button"
